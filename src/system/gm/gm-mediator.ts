...
export class GmMediator extends Mediator {
    ...
    onAwake(): void {
        ...
        this.owner.useBtn.on(Laya.Event.CLICK,()=>{
            app.gmd.callGm({cmd:this.owner.textInput.text})
        })
        
        this.owner.resetBtn.on(Laya.Event.CLICK,()=>{
            app.gmd.callGm({cmd:"reset"})
        })
        this.updateList()
    }
    ...
    updateList(){
        ...
        this.listData.push({name:"发送自定义邮件",instruct:"gm_mail hello 'content xxx' {10101,11},{10102,22}}"})
